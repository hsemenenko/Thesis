...
plot([dt0, dt25, dt100, dt250],
    yaxis = ("Detection Probability", :log10, (1.5e-3,2e-1)),
...
coin2 = [k^2 for k in [find_zero(x -> f(l, 25, 0.2, x), (0,1)) for l in 0:1:350]]
coin4 = [4*k^2*(1-k)^2 for k in [find_zero(x -> f(l, 25, 10^(-0.3) * 0.2, x), (0,1)) for l in 0:1:350]]
coin8 = [16*k^2*(1-k)^6 for k in [find_zero(x -> f(l, 25, 10^(-0.6) * 0.2, x), (0,1)) for l in 0:1:350]]
coin16 = [64*k^2*(1-k)^14 for k in [find_zero(x -> f(l, 25, 10^(-0.9) * 0.2, x), (0,1)) for l in 0:1:350]]
coin32 = [256*k^2*(1-k)^30 for k in [find_zero(x -> f(l, 25, 10^(-1.2) * 0.2, x), (0,1)) for l in 0:1:350]]
coin64 = [1024*k^2*(1-k)^30 for k in [find_zero(x -> f(l, 25, 10^(-1.5) * 0.2, x), (0,1)) for l in 0:1:350]]

plot(dt0.^2,
    yaxis = ("Probability of Coincidence", :log10, (2e-5, 5e-2), [1e-2, 1e-3, 1e-4]),
    xaxis = ("Distance", (0,150)),
    label = "",
    ls = :dash,
