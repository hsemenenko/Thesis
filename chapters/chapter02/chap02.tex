...
\brisred{Alice}, for reasons that we will not discuss here, would like to send a message to Bob without any other party knowing the message. While she has never met Bob, she wants to make sure that the message is actually being sent to Bob.

\brisred{Bob} wants to receive and read the message from Alice, whom he has never met, whilst also being sure that the message is, in fact, from Alice. He also wants to ensure that the message has not been tampered with along the way.

\brisred{Charlie} is a friend of Alice and Bob who acts as a mediator. His role will depend on how they are communicating, but he is often forgotten. While he is not actively trying to sabotage Alice and Bob's communication, neither is he actively protecting them.

\brisred{Eve} is a very nosey individual who listens to all public communication between Alice and Bob. She is not very inventive or active so will not put very much effort into deciphering the message. Eve is often mistaken as Mallory, as they have similar intents.

\brisred{Mallory} would also like to know what Alice and Bob are saying to each other, but will go to more extreme measures to discover their secrets. She has unlimited resources available (confined only by the laws of physics) and will use all possible methods within her power to crack the ciphertext. 

\section{Cryptography}
\label{sec1:crypto}

Cryptography is the process of taking a message (\brisred{plaintext}) and translating it into a \brisred{ciphertext}. Importantly, it also encompasses the reverse process of taking a ciphertext and returning the original message. The idea of obscuring messages from third-party onlookers dates back (as far as we can tell) to ancient Egypt and has played an important role in the world for the last 2 millennia. 
...
One of the most vulnerable parts of a \ac{QKD} system are the detectors. Due to their complexity, they have been exposed to many attacks which have allows an attacker to be 
