...
Integrated quantum photonics has facilitated a drastic increase in complexity of experiments simply not possible with alternatives. The inherent phase stability is a vital resource for quantum experiments that are much more sensitive to small fluctuations. 
...
\begin{equation}
	\mathcal{E}_j^\text{in}(t, t_p, \omega_j, \phi_j) = \sqrt{\frac{2 \sqrt{2\ln(2)}}{t_p\sqrt{\pi}}} \exp{\left(-\frac{4 \ln(2) t^2}{t_p^2}\right)} \exp{\left(i(\omega_j t + \varphi_j)\right)}
\end{equation}
...
By substituting the Gaussian pulses from above into the beam splitter relationships to get the electric fields after interference. After integrating over the pulses in time (which is equivalent to the convolution of the two functions in frequency space), we can calculate the intensities of the output electric fields

\begin{align}
	\mathcal{I}^\text{out}_1 &= 1 + \exp\left(-\frac{t_p^2(\omega_1 - \omega_2)^2}{32\ln(2)}\right)\cos(\Delta\phi)\\
	\mathcal{I}^\text{out}_2 &= 1 - \exp\left(-\frac{t_p^2(\omega_1 - \omega_2)^2}{32\ln(2)}\right)\cos(\Delta\phi)
\end{align}
...
\begin{equation}
	\langle \mathcal{I}^\text{out}_1 \mathcal{I}^\text{out}_2\rangle = \frac{1}{2\pi}\int_0^{2\pi} 1 - \exp\left(-\frac{t_p^2 \Delta\omega^2}{16\ln(2)}\right)\cos(\Delta\phi)^2 d(\Delta\phi)
\end{equation} 
...
In this sections we will describe the integrated transmitters that we will use for generating weak coherent states. Each transmitter is \ac{InP} device, measuring \SI{6x2}{mm}, that was fabricated by Oclaro. 
...
These lasers have demonstrated a coherence time of \SI{1.5}{ns} \cite{Sibson2017InP} which corresponds to a \ac{FWHM} of \SI{1.7}{pm}. We will see later that partial coherence can be seen at \SI{4}{ns} which provides a lower bound on the \ac{FWHM} of around \SI{0.7}{pm}\footnote{The coherence length of the laser can be calculated from the linewidth by $L_\text{coh} = \frac{c}{\pi\Delta v}$ where $\Delta v$ is the \ac{FWHM} of the laser in frequency.}.
...
To be able to access the electrical components of the integrated devices, we need to create a package to transfer the signals from the driving electronics to the chip . A PCB was designed to electrically connect the modulators and laser to SMA and DC pin connectors. A render is shown in figure \ref{fig:chip_pcb}. The bottom of the chip is ground which connects to a copper block that is also connected to the ground of the PCB. Silver epoxy was used to glue the chip to the mount to ensure a good electrical connection. It also provided a good thermal connection for temperature stabilisation.

In order to maintain a good signal integrity the impedance of the tracks needs to match the electronics. As the devices require a number of RF connections to a small area, using standard FR-4 material would not be sufficient. Instead, Rogers 6006ns material was chosen for its high dielectric constant of \num{6.15} meaning that thin grounded coplanar waveguides with ground could still maintain \SI{50}{\ohm} impedance. A schematic of the waveguides is shown in figure \ref{fig:GSG_waveguide}. The characteristic impedance of a grounded coplanar waveguide can be approximated by \cite{wadell1991}
\begin{equation}
	Z_0 = \frac{60\pi}{\sqrt{E_\text{eff}}}\frac{1}{\frac{K(k)}{K(k')} + \frac{K(k_l)}{K(k_l')}}
\end{equation}
where, for a track of width $a$, waveguide of width $b$ and substrate of height $h$,
\begin{align}
...
In figure \ref{fig:gain_switch}, we demonstrate gain switching the on-chip laser by providing a \SI{-1.5}{V} RF signal. This drains the cavity so that when lasing resumes the phase will come from a spontaneous emission and will be random compared with the previous window. 

After the negative pulses, the lasing oscilates before returning to a continuous (and useful) lasing mode. These oscillations can be attributed to mode competition in the laser cavity and also RF signal ringing.  
