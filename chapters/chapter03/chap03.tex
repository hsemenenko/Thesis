...
In order to maintain a good signal integrity the impedance of the tracks needs to match the electronics. As the devices require a number of RF connections to a small area, using standard FR-4 material would not be practical. Instead, Rogers 6006ns material was chosen for its high dielectric constant of \num{6.15} meaning that thin coplanar waveguides with ground could still maintain \SI{50}{\ohm} impedance.

The characteristic impedance of a coplanar waveguide with ground can be approimated by \cite{wadell1991}
\begin{equation}
...
\begin{align}
	...
	k' &= \sqrt{1 - k^2}\\
	k_l' &= \sqrt{1 - k_l^2}\\
	k_l &= \frac{\tanh\left(\frac{\pi a}{4 h}\right)}{\tanh\left(\frac{\pi b}{4 h}\right)}
\end{align}
and
\begin{equation}
...
\begin{figure}[t]
	...
	\caption[Ground-Signal-Ground coplanar waveguide]{Ground-Signal-Ground coplanar waveguide}
